...
class EditFamilyNamePage extends StatefulWidget {
  ...
  const EditFamilyNamePage(
      {Key? key, required this.currentname, required this.familyid})
      : super(key: key);
          ...
          child: Container(
            ...
            child: ElevatedButton(
              ...
              onPressed: isButtonEnabled
                  ? () async {
                      if (await editNamesendData(name, widget.familyid) ==
                          200) {
...
editNamesendData(String name, int id) async {
  final storage = FlutterSecureStorage();
  final accessToken = await storage.read(key: 'ACCESS_TOKEN');
  var dio = await authDio();
  dio.options.headers['Authorization'] = '$accessToken';
  final response = await dio.patch('/api/v1/family/$id/nickname',
      data: jsonEncode({'nickname': name}));
